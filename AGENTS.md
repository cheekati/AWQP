...
- The Web dashboard loads Bootstrap/jQuery from public CDNs (`cdn.jsdelivr.net`, `code.jquery.com`). In a sandbox without outbound internet the page content/data still render correctly but appear **unstyled** — this is a network limitation, not a broken build. The local `~/css/site.css` is served by the app itself and is unaffected.
